...
#Playground {
    ...
    let backendService = CandidateBackendService(apiService: apiService, keychainService: keychainService)
    
    print("📦 Services initialisés.")
    
    // 2. Authentification (Vraie requête réseau)
    print("🔑 Tentative de connexion (admin@vitesse.com)...")
    
    try await backendService.userAuthenticate(email: "admin@vitesse.com", password: "test123")
    
    let isAdmin = await backendService.isAdmin
    print("✅ Connecté ! (Admin: \(isAdmin == true))")
    
    // 3. Préparation du candidat
    let newCandidate = CandidateRequest(
        firstName: "Sarah",
        lastName: "Conhor",
        email: "sarah.\(Int.random(in: 1...1000))@conhor.com", // Email unique pour éviter les doublons
        phone: "0102030405",
        linkedinURL: "https://linkedin.com/in/sarah-conhord",
        note: nil
    )
    
    // 4. Création (Vraie requête réseau)
    print("📤 Envoi de la création du candidat...")
    
    let createdCandidate = try await backendService.createCandidate(candidateRequest: newCandidate)
    
    print("""
                
                🎉 SUCCÈS ! Candidat créé :
                ---------------------------
                🆔 ID    : \(createdCandidate.id)
                👤 Nom   : \(createdCandidate.firstName) \(createdCandidate.lastName)
                📧 Email : \(createdCandidate.email)
                📝 Note  : \(createdCandidate.note ?? "")
                ---------------------------
                """)
}
